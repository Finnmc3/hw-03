...
emailhr = 0.69*educ-2.76
Higher the education level, the more time spent emails.
...
A rend can be seen in the plot of the residuals of a line with a negative gradient. This shows that a different might give a better performance.

```{r}
emailhr_educ_model <- linear_reg() %>%
  set_engine("lm") %>%
  fit(emailhr ~ educ, data = gss16_advfront)
  tidy(emailhr_educ_model)

#emailhr = 0.69*educ-2.76
#Higher the education level, the more time spent emails.


emailhr_educ_model_aug <- augment(emailhr_educ_model$fit)

ggplot(emailhr_educ_model_aug, mapping = aes(x = .fitted, y = .resid)) +
...
A logistic regression model is more suited for using binary data to make predictions.
...
```{r}
gss16_pred_1 <- predict(gss16_fit_1, gss16_train, type = "prob") %>%
  bind_cols(gss16_train)
...
The second model since the ROC curve is closer to the point (0,1) indicating that has a more accurate prediction than the first model

```{r}
gss16_pred_2 <- predict(gss16_fit_2, gss16_train, type = "prob") %>%
  bind_cols(gss16_train)
